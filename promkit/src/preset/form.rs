            ...
            states.into_iter().fold(
                ...
                |(mut readlines, mut focus_styles, mut unfocus_styles), state| {
                    let focus_style = Style {
                        prefix_style: state.formatter.prefix_style,
                        active_char_style: state.formatter.active_char_style,
                        inactive_char_style: state.formatter.inactive_char_style,
                    };

                    let unfocus_style = Style {
                        prefix_style: ContentStyle {
                            attributes: Attributes::from(Attribute::Dim),
                            ..state.formatter.prefix_style
                        },
                        ...
                        inactive_char_style: ContentStyle {
                            attributes: Attributes::from(Attribute::Dim),
                            ..state.formatter.inactive_char_style
                        },
                    };
        ...
        self.readlines
            ...
            .for_each(|(i, state)| {
                if i == current_position {
                    state.formatter.prefix_style = self.focus_styles[i].prefix_style;
                    state.formatter.inactive_char_style = self.focus_styles[i].inactive_char_style;
                    state.formatter.active_char_style = self.focus_styles[i].active_char_style;
                } else {
                    state.formatter.prefix_style = self.unfocus_styles[i].prefix_style;
                    state.formatter.inactive_char_style =
                        self.unfocus_styles[i].inactive_char_style;
                    state.formatter.active_char_style = self.unfocus_styles[i].active_char_style;
                }
            });
    }
}
